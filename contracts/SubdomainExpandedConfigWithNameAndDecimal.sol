...
// Deployed by 0x6a6418fB7980D92F664d6583e0AeaF7740bf8eeE on 12-24-21, compiled with 0.8.4
...
contract SubdomainConfig is Ownable {
    ...
    function configureSubdomainFullyWithVersion(
        bytes32 tickerLabel,
        address owner,
        bytes32 fullNameNode,
        address mainnetTokenAddress,
        string[6] calldata metadata
    ) onlyOwner public {
        
        ensRegistry.setSubnodeRecord(TKR_NAMEHASHED_NODE, tickerLabel, address(this), address(publicResolver), 0);
        publicResolver.setAddr(fullNameNode, mainnetTokenAddress);
        publicResolver.setText(fullNameNode, "name", metadata[0]);
        publicResolver.setText(fullNameNode, "url", metadata[1]);
        publicResolver.setText(fullNameNode, "avatar", metadata[2]);
        // publicResolver.setText(fullNameNode, "description", description);
        publicResolver.setText(fullNameNode, "decimals", metadata[3]);
        publicResolver.setText(fullNameNode, "version", "0.0.1");
        publicResolver.setText(fullNameNode, "com.twitter", metadata[4]);
        publicResolver.setText(fullNameNode, "com.github", metadata[5]);
        ensRegistry.setSubnodeOwner(TKR_NAMEHASHED_NODE, tickerLabel, owner);
    }
